...
class NodeService():
    ...
    def join_network(self, payload: dict, host: str):
        ...
        response = requests.post(
            f"http://{addr}/node/register", data=json.dumps(data), headers=headers)
        if response.status_code == 200:
            payload = response.json()
            # update chain and the peers
            self.library.createBlockchain(
            ).create_chain_from_dump(payload['chain'])
            # #nesse nó ou em um novo nó DESENHAR!
            self.library.createPeersManager().sync_peers(
                [addr, *payload['peers']], host)
    ...
    def sync_block(self, block: dict):
        ...
        if not added:
            return Response({"message": "The block is discarded by the node."}, status.HTTP_500_INTERNAL_SERVER_ERROR)
